var items = new Firebase("https://easyauction.firebaseio.com/items")
...
function showAllItems() {
  items.on("value",readItems,function (e){
    console.log (e);
  });
}
...
function updateItem(title, price, descrip, pic) {
  ...
  curr.update({"title":title, "price":price, "descrip":descrip, "imgD":pic, "userTime": new Date($.now()).toLocaleString()})
}

function selectExpItems(e) {
  $("#items").empty();
  items.orderByChild("price").startAt(10000).on("child_added",reArrangeItems);
}

function selectCheapItems() {
  $("#items").empty();
  items.orderByChild("price").endAt(9999).on("child_added",reArrangeItems);
}

function removeItems() {
  getItemByURL("items/"+ nowItem).remove();
  getItemByURL("users/"+ users.getAuth().uid+ "/sellItems/"+ nowItem).remove();
}
...
function authenticate() {
  users.authWithOAuthPopup("facebook",function (error, authData) {
    if (error) {
      ...
      users.orderByKey().equalTo(authData.uid).once("value",function (snapshot) {
        if (!snapshot.val()) {
          var obj = {} ;
          obj[authData.uid] = {
            name: authData.facebook.displayName,
            pic: authData.facebook.profileImageURL
          };
          users.update(obj);
        }
      })
...
function getItemByURL(suburl) {
  return new Firebase("https://easyauction.firebaseio.com/"+suburl);
}
